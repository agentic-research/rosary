...
rsry hooks install / status                  # install/report the bead-sync hooks (ADR-0024 amendment A — how a bead reaches git): a store write never touches `.beads/beads.jsonl`; commit-msg publishes the beads the subject names into the tracked export and post-commit folds that record into the commit (rosary-e5bfd3); pre-push (`bead verify-pushed`, rosary-e5c037) gates the PUSHED tip's blob for the beads the pushed commits name — never the working tree, a code-only push is never refused; post-merge imports the merged export, runs `close-merged --local`, then `bead trunk-refresh --push` on the trunk (rosary-e5c0a0): re-render every published bead, commit, push — a refused push (PR-only trunk) parks the commit on `rsry/trunk-refresh` until the trunk writer is decided (rosary-e5be46). `rsry bead publish <ids>` is the explicit hand. Under a global `core.hooksPath` per-repo install is skipped (rosary-590ddf)
rsry hooks audit                             # mechanical gate (exit non-zero on failure): .gitignore shadowing beads.jsonl, .beads/embeddeddolt+beads.db backend ambiguity, TRUNK projection vs store record-by-record (the same comparison pre-push makes; a feature branch's file lagging the store is not drift — rosary-e5fc0e), stale installed hook stamps, foreign-repo-shaped committed dependencies (rosary-d93ab7)
