...
rsry hooks install / status                  # install/report the pre-commit/commit-msg/post-commit/post-merge/post-push bead-sync hooks (commit-msg publishes the beads the subject names into the tracked export and post-commit folds the record into that commit — rosary-e5bfd3; post-merge runs `close-merged --local`) + the `beads-jsonl` merge driver config + the pre-push hard gate (`bead verify-pushed`, rosary-e5c037): for the beads the pushed commits name in their subjects, the pushed tip's `.beads/beads.jsonl` must carry the store's exact record — gates the artifact, not the working tree; a code-only push is never refused
