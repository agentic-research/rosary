...
test ! -e "$legacy_tunnel_plist" ||
  fail_rsry_service_installer_test "legacy Homebrew-backed tunnel plist survived installation"
...
if grep -q "/opt/homebrew/bin" "$canonical_plist"; then
  fail_rsry_service_installer_test "canonical plist still exposes Homebrew binaries to rsry"
fi

legacy_unload_line="$(
  grep -nF "unload $legacy_plist" "$launchctl_log" | head -n1 | cut -d: -f1
)"
legacy_tunnel_unload_line="$(
  grep -nF "unload $legacy_tunnel_plist" "$launchctl_log" | head -n1 | cut -d: -f1
)"
canonical_load_line="$(
...
test -n "$legacy_tunnel_unload_line" ||
  fail_rsry_service_installer_test "legacy Homebrew-backed tunnel was not unloaded"
...
test "$legacy_tunnel_unload_line" -lt "$canonical_load_line" ||
  fail_rsry_service_installer_test "canonical service loaded before tunnel cleanup"
...
test ! -e "$legacy_tunnel_plist" ||
  fail_rsry_service_installer_test "second installation recreated the legacy tunnel plist"
