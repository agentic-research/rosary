...
2. **rosary-9c4c7a** (P1) — bdr channels/harmony + reconcile/completion: event bus + bead correlation
3. **rosary-e13ecf** (P1) — `accrete.rs`: wire accrete() into reconciler on close
4. **rosary-fdb65b** (P1) — decompose/import: auto-decompose external issues
5. **rsry-c1a52f** (P1) — `parse.rs`: mache schema-driven doc classification
6. **rsry-eb0b83** (P1) — `e2e-sandbox.sh`: automated no-HITL dispatch-verify-close loop
7. **rosary-42eccd** (P2) — `observation/resolve.rs`: verify undercut proof
8. **rosary-b6367a** (P2) — `reconcile/verify.rs`: mechanical merge gate
...
2. **Fix the 86 stale scopes** (mechanical, MCP-gated on rosary-080934) — this
   widens the lanes by pulling beads out of the phantom `src/dispatch.rs` cluster.
3. **Keep decomposing the god-files** (handlers.rs/main.rs) — the only way to
   shrink the 130-cluster structurally. Parallelism is a *decomposition* outcome,
   not a *scheduling* one.
4. Cross-repo beads (signet/ll/conductor) living in the rosary store are a
