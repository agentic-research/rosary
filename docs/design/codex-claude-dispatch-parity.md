...
1. Extract a provider-neutral run/session/event/grant contract.
1. Build deterministic fake-agent harness against that contract.
1. Port Claude/ACP through the contract without changing behavior.
1. Add native Codex provider using Codex-native thread/session APIs.
1. Add ignored live Codex dispatch smoke.
1. Dogfood fan-out with one bead per Codex thread/workspace.
