...
1. **conductor (Elixir/OTP)** — agent lifecycle supervision, ACP protocol, pipeline progression
...
1. For each, check if the agent process is still alive (PID check)
1. If alive, re-attach supervision
1. If dead, decide: resume (next phase) or retry (same phase)
...
1. Write PipelineState to backend Dolt
1. Spawn agent
...
1. Read `list_active_pipelines()` from backend
1. For each pipeline, read the bead's actual status from rsry
1. Cross-reference:
...
1. **Keep `rsry run`?** Yes, as a single-process fallback for local dev. Both paths share bead state validation — `persist_status()` enforces `valid_transitions()` regardless of caller.

1. **Conductor triage?** Conductor should consume rsry's triage scores via `rsry_list_dispatchable` (Phase 3), not reimplement.

1. **Verification ownership?** Conductor owns verification (it has the work directory and exit status). Verification results are persisted via rsry (bead comments, status transitions).
...
1. **RsryClient bottleneck**: The GenServer serializes all MCP calls. At 5+ concurrent agents this becomes a bottleneck. Direct `Req` from each AgentWorker, or connection pool?
