...
1. **Synthesize** — amalgamate findings into unified understanding
1. **Derive** — extract actionable work for next sprint
1. **Decompose** — break into parallelizable, file-per-agent tasks
1. **Dispatch** — launch agents with appropriate permissions
1. **Verify** — check work, close beads, update docs
...
1. **Dependency analysis**: which beads are blocked by others?
1. **Composition detection**: do any new beads subsume existing ones?
1. **Dedup check**: are any beads saying the same thing?
1. **Parallelization**: group beads by primary file to maximize agent independence
1. **Permission derivation**: map issue_type → PermissionProfile for each task
