...
1. **Coupling**: Every new "reaction" (sync to Linear, advance pipeline, notify agent) requires adding code to the reconciler's polling loop.
1. **Scale ceiling**: Dolt serializes all writes at ~300/sec. With 50-200 concurrent agents, this becomes a bottleneck.
...
1. **Subscription/notification** — generation counter detects changes but no pub/sub for "bead X changed." Add SQLite `update_hook` or channel-based notification.
1. **Multi-writer support** — single `Mutex<SqliteGraph>` writer. For 50+ agents, need concurrent writes or write-through-Elixir serialization.
1. **Dolt bridge** — beads currently live in Dolt. Either ley-line reads/writes Dolt, or beads migrate into ley-line's SQLite arena with Dolt as federation export.
...
1. Add `IssueTracker::update_fields()` to trait (backend-agnostic field sync)
1. Prototype ley-line write path for bead state (new `nodes` schema or separate `beads` table in arena)
1. Add subscription mechanism (SQLite `update_hook` -> channel -> Elixir)
1. Dual-write: MCP tools write Dolt + ley-line during transition
1. Validate reactive pipeline: webhook -> ley-line write -> conductor dispatch -> agent -> progress -> Linear
1. Register rosary as Linear Agent (OAuth app, webhook handler)
1. If proven: ley-line becomes primary, Dolt becomes federation export
