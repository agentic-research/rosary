...
1. **Simplify ruthlessly.** If you can delete code and the tests still pass, delete
   it. If an abstraction has one caller, inline it. If a file does two things, split
   it. Complexity is debt.

1. **Make the implicit explicit.** If a pattern exists but isn't named, name it.
...
1. **The dependency graph is the architecture.** Before changing anything, understand
...
1. **Tests are specifications.** A test that doesn't describe intended behavior is
   noise. Delete noisy tests. Write tests that would fail if the behavior changed
   in ways users would notice.

1. **Creative solutions welcome.** The best code isn't the most "correct" — it's the
