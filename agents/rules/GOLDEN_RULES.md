...
## 12. Question Before You Build

Before any implementation bead is dispatched, answer these five questions:

1. **WHO** is this for? (hosted user, CLI user, agent, operator)
1. **WHAT** does the onboarding look like? (zero-step, one-step, multi-step)
1. **WHEN** does this become a problem? (measurable trigger, not "eventually")
1. **HOW MUCH** does this cost? (infra cost, complexity cost, maintenance cost)
1. **WHAT'S THE BLAST RADIUS?** (who/what breaks if this is wrong?)

If any answer is "I don't know" → create a research bead first, not an implementation bead.
...
1. Does the fix violate a Golden Rule? If yes, tag the waiver: `--labels "waiver:rule-<number>,reason:<why>"`
1. If unsure whether a rule applies, don't tag it — false positives are worse than missed tags
