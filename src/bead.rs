...
// rosary-2643b6: the close-condition family lives in its own module now —
// re-exported so every existing `crate::bead::X` call site keeps resolving
// unchanged. DEFAULT_PR_MERGE_CLOSE_CONDITION/requires_close_condition are
// re-export-only from this file's own perspective (used elsewhere in the
// crate, not unqualified here) — hence the allow.
#[allow(unused_imports)]
pub use crate::bead_close_condition::{
    DEFAULT_PR_MERGE_CLOSE_CONDITION, ensure_close_condition, has_close_condition,
    merge_alone_satisfies_close, requires_close_condition, resolve_acceptance_criteria,
};
